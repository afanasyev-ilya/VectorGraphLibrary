...
void BFS::nec_top_down(VectCSRGraph &_graph,
                       VerticesArray<_T> &_levels,
                       VerticesArray<_T> &_parents,
                       int _source_vertex)
{
    VerticesArray<VGL_PACK_TYPE> packed_data(_graph);
    GraphAbstractionsNEC graph_API(_graph);
    FrontierNEC frontier(_graph);
    ...
    auto init_levels = [&_levels, &_parents, _source_vertex] (int src_id, int connections_count, int vector_index)
    {
        if(src_id == _source_vertex)
        {
            _levels[_source_vertex] = FIRST_LEVEL_VERTEX;
            _parents[_source_vertex] = _source_vertex;
        }
        else
        {
            _levels[src_id] = UNVISITED_VERTEX;
    ...
    frontier.set_all_active();
    graph_API.compute(_graph, frontier, init_levels);
    graph_API.pack_vertices_arrays(packed_data, _levels, _parents);
    ...
    while(frontier.size() > 0)
    {
        auto edge_op = [&_levels, &_parents, current_level](int src_id, int dst_id, int local_edge_pos,
                long long int global_edge_pos, int vector_index, DelayedWriteNEC &delayed_write)
        {
            _T dst_level = _levels[dst_id];
            if(dst_level == UNVISITED_VERTEX)
        ...
        auto packed_edge_op = [&packed_data, current_level](int src_id, int dst_id, int local_edge_pos,
                long long int global_edge_pos, int vector_index, DelayedWriteNEC &delayed_write)
        {
            VGL_PACK_TYPE packed = packed_data[dst_id];
            _T dst_level, dst_parent;
            vgl_unpack(packed, dst_level, dst_parent);

            if(dst_level == UNVISITED_VERTEX)
            {
                vgl_pack(packed_data[dst_id], current_level + 1, src_id);
            }
        };

        graph_API.scatter(_graph, frontier, packed_edge_op, EMPTY_VERTEX_OP, EMPTY_VERTEX_OP,
                          packed_edge_op, EMPTY_VERTEX_OP, EMPTY_VERTEX_OP);
        ...
        auto packed_on_next_level = [&packed_data, current_level] (int src_id, int connections_count)->int
        {
            VGL_PACK_TYPE packed = packed_data[src_id];
            _T src_level, src_parent;
            vgl_unpack(packed, src_level, src_parent);

            int result = NOT_IN_FRONTIER_FLAG;
            if(src_level == (current_level + 1))
                result = IN_FRONTIER_FLAG;
            return result;
        };

        graph_API.generate_new_frontier(_graph, frontier, packed_on_next_level);
