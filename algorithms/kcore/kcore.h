...
public:
    ...
    static void calculate_kcore_sizes(ExtendedCSRGraph &_graph,
                                      int *_kcore_data,
                                      int &_vertices_count,
                                      long long &_edges_count);
    
    // returns int *_kcore_degrees array of size |V|, i-th vertex in kcore <=> _kcore_degrees[i] > 0
    static void kcore_subgraph(ExtendedCSRGraph &_graph, int *_kcore_degrees, int _k);
    static void kcore_subgraph(VectorisedCSRGraph &_graph, int *_kcore_degrees, int _k);
    
    static void maximal_kcore(ExtendedCSRGraph &_graph, int *_kcore_degrees);
};
