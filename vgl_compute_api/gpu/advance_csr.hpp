...
template <typename EdgeOperation, typename VertexPreprocessOperation,
        typename VertexPostprocessOperation>
void __global__ vg_csr_advance_warp_per_vertex_kernel(long long *_vertex_pointers,
                                                      int *_adjacent_ids,
                                                      int *_frontier_ids,
                                                      int _frontier_size,
                                                      size_t _process_shift,
                                                      ...
                                                      VertexPostprocessOperation vertex_postprocess_op)
{
    const register int warps_per_block = blockDim.x / WARP_SIZE;
    const register int wid = warp_id();
    const register int frontier_pos = blockIdx.x * warps_per_block + wid;
    const register int lid = lane_id();
    ...
    if(frontier_pos < _frontier_size)
    {
        ...
        for(int local_edge_pos = lid; local_edge_pos < connections_count; local_edge_pos += WARP_SIZE)
        {
            const long long internal_edge_pos = start + local_edge_pos;
            const int vector_index = lane_id();
            const int dst_id = _adjacent_ids[internal_edge_pos];
