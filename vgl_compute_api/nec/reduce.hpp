...
void GraphAbstractionsNEC::reduce_worker_sum(GraphContainer &_graph,
...
template <typename _T, typename ReduceOperation, typename GraphContainer, typename FrontierContainer>
void GraphAbstractionsNEC::reduce_worker_max(GraphContainer &_graph,
                                             FrontierContainer &_frontier,
                                             ReduceOperation &&reduce_op,
                                             _T &_result)
{
    ...
    if(_frontier.get_sparsity_type() == ALL_ACTIVE_FRONTIER)
    {
        ...
        #pragma _NEC novob
        #pragma _NEC vector
        #pragma omp parallel for schedule(static) reduction(max: reduce_result)
        for(int src_id = 0; src_id < frontier_size; src_id++)
        {
            int connections_count = _graph.get_connections_count(src_id);
            int vector_index = get_vector_index(src_id);
            _T val = reduce_op(src_id, connections_count, vector_index);
            reduce_result = vect_max(val, reduce_result);
        }
    }
    else if(_frontier.get_sparsity_type() == DENSE_FRONTIER)
    {
...
template <typename _T, typename ReduceOperation, typename GraphContainer, typename FrontierContainer>
void GraphAbstractionsNEC::reduce_worker_min(GraphContainer &_graph,
                                             FrontierContainer &_frontier,
                                             ReduceOperation &&reduce_op,
                                             _T &_result)
{
    ...
    if(_frontier.get_sparsity_type() == ALL_ACTIVE_FRONTIER)
    {
        ...
        #pragma _NEC novob
        #pragma _NEC vector
        #pragma omp parallel for schedule(static) reduction(min: reduce_result)
        for(int src_id = 0; src_id < frontier_size; src_id++)
        {
            int connections_count = _graph.get_connections_count(src_id);
            int vector_index = get_vector_index(src_id);
            _T val = reduce_op(src_id, connections_count, vector_index);
            reduce_result = vect_min(val, reduce_result);
        }
    }
    else if(_frontier.get_sparsity_type() == DENSE_FRONTIER)
    {
...
template <typename _T, typename ReduceOperation, typename GraphContainer, typename FrontierContainer>
void GraphAbstractionsNEC::reduce_worker(GraphContainer &_graph,
                                         FrontierContainer &_frontier,
                                         ReduceOperation &&reduce_op,
                                         ...
                                         _T &_result)
{
    ...
    {
        reduce_worker_min(_graph, _frontier, reduce_op, _result);
    }
    else if(_reduce_type == REDUCE_AVG)
    {
        reduce_worker_sum(_graph, _frontier, reduce_op, _result);
        _result = _result / _frontier.get_size();
    }
    else
