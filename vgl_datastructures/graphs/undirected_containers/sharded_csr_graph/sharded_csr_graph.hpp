...
ShardedCSRGraph::ShardedCSRGraph(SupportedDirection _supported_direction)
{
    ...
    this->edges_count = 1;
    this->graph_format = SHARDED_CSR_GRAPH;
...
bool ShardedCSRGraph::save_to_binary_file(string _file_name)
{
    ...
    fwrite(reinterpret_cast<const char*>(&(this->graph_format)), sizeof(GraphStorageFormat), 1, graph_file);
...
bool ShardedCSRGraph::load_from_binary_file(string _file_name)
{
    ...
    fread(reinterpret_cast<char*>(&(this->graph_format)), sizeof(GraphStorageFormat), 1, graph_file);
    if(this->graph_format != SHARDED_CSR_GRAPH)
    {
