...
def get_list_of_synthetic_graphs(run_speed_mode):
    if run_speed_mode == "tiny-only":
        ...
        return syn_scaling
    elif run_speed_mode == "great":
        return syn_great
    elif run_speed_mode == "gpu_bfs_ml":
        return []
    elif run_speed_mode == "cpu_bfs_ml":
        return []
    elif run_speed_mode == "gpu_pr_ml":
        return []
    elif run_speed_mode == "cpu_pr_ml":
        return []
    elif run_speed_mode == "gpu_cc_ml":
        return []
    elif run_speed_mode == "cpu_cc_ml":
        return []
    elif run_speed_mode == "gpu_sssp_ml":
        return []
    elif run_speed_mode == "cpu_sssp_ml":
        return []
...
def get_list_of_real_world_graphs(run_speed_mode):
    if run_speed_mode == "tiny-only":
        ...
        return list(great_graphs_data.keys())
    elif run_speed_mode == "gpu_bfs_ml":
        return gpu_format_ml
    elif run_speed_mode == "cpu_bfs_ml":
        return cpu_format_ml
    elif run_speed_mode == "gpu_pr_ml":
        return gpu_format_ml_pr
    elif run_speed_mode == "cpu_pr_ml":
        return cpu_format_ml_pr
    elif run_speed_mode == "gpu_cc_ml":
        return gpu_format_ml_cc
    elif run_speed_mode == "cpu_cc_ml":
        return cpu_format_ml_cc
    elif run_speed_mode == "gpu_sssp_ml":
        return gpu_format_ml_sssp
    elif run_speed_mode == "cpu_sssp_ml":
        return cpu_format_ml_sssp
        return 
    else:
        raise ValueError("Unsupported run_speed_mode")
...
def get_list_of_all_graphs(run_speed_mode):
    if run_speed_mode == "great":
        return great_graphs_data.keys()
    else:
...
def download_all_real_world_graphs(run_speed_mode):
    ...
    if run_speed_mode != "great":
        ...
        download_graph_great()
         
            

def download_graph_great():
    for graph_name in great_graphs_data.keys():
        file_name = SOURCE_GRAPH_DIR + "download.tsv." + great_graphs_data[graph_name]["link"] + ".tar.bz2"
        if not path.exists(file_name):
            link = "http://konect.cc/files/download.tsv." + great_graphs_data[graph_name]["link"] + ".tar.bz2"
            ...
            print("File " + SOURCE_GRAPH_DIR + "/" + great_graphs_data[graph_name]["link"] + ".tar.bz2" + " exists!")

def download_graph_great_cur(graph_name):
...
def great_create_real_world_graph(graph_name, arch):
    graph_format = "el_container"
    output_graph_file_name = get_path_to_graph(graph_name, graph_format)
    if not file_exists(output_graph_file_name):
        download_graph_great_cur(graph_name)

        tar_name = SOURCE_GRAPH_DIR + "download.tsv." + great_graphs_data[graph_name]["link"] + ".tar.bz2"
        cmd = ["tar", "-xjf", tar_name, '-C', "./bin/source_graphs"]
        subprocess.call(cmd, shell=False, stdout=subprocess.PIPE)

        if "unarch_graph_name" in great_graphs_data[graph_name]:
            source_name = "./bin/source_graphs/" + great_graphs_data[graph_name]["link"] + "/out." + great_graphs_data[graph_name]["unarch_graph_name"]
        else:
            source_name = "./bin/source_graphs/" + great_graphs_data[graph_name]["link"] + "/out." + great_graphs_data[graph_name]["link"]
...
def create_graph(graph_name, arch, run_speed_mode):
    if run_speed_mode == "great":
        great_create_real_world_graph(graph_name, arch)
    else:
