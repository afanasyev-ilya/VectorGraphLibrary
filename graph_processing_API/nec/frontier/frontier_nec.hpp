...
#ifdef __USE_MPI__
void FrontierNEC::calculate_vector_engine_mpi_thresholds()
{
    ...
    vector_engine_mpi_thresholds = current_direction_graph->get_mpi_thresholds(vgl_library_data.get_mpi_rank(), 0, vector_engine_part_size);
}
#endif
...
#ifdef __USE_MPI__
void FrontierNEC::calculate_vector_core_mpi_thresholds()
{
    UndirectedCSRGraph *current_direction_graph;
    VectCSRGraph *vect_csr_ptr = (VectCSRGraph*)graph_ptr;
    current_direction_graph = vect_csr_ptr->get_direction_graph_ptr(direction);

    vector_core_mpi_thresholds = current_direction_graph->get_mpi_thresholds(vgl_library_data.get_mpi_rank(), vector_engine_part_size,
...
#ifdef __USE_MPI__
void FrontierNEC::calculate_collective_mpi_thresholds()
{
    UndirectedCSRGraph *current_direction_graph;
    VectCSRGraph *vect_csr_ptr = (VectCSRGraph*)graph_ptr;
    current_direction_graph = vect_csr_ptr->get_direction_graph_ptr(direction);

    collective_mpi_thresholds = current_direction_graph->get_mpi_thresholds(vgl_library_data.get_mpi_rank(),
                                                       vector_engine_part_size + vector_core_part_size,
