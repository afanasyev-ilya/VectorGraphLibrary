...
void GraphPrimitivesNEC::advance_worker(EdgesListGraph<_TVertexValue, _TEdgeWeight> &_graph,
                                        EdgeOperation &&edge_op)
{
    ...
    for(long long vec_start = 0; vec_start < edges_count - VECTOR_LENGTH; vec_start += VECTOR_LENGTH)
    {
        #pragma _NEC ivdep
    ...
    #pragma omp single
    {
        ...
        #pragma _NEC vector
        #pragma _NEC gather_reorder
        for(int i = (edges_count - VECTOR_LENGTH); i < edges_count; i ++)
        {
            long long global_edge_pos = i;
            int vector_index = i - (edges_count - VECTOR_LENGTH);
            int src_id = src_ids[global_edge_pos];
            int dst_id = dst_ids[global_edge_pos];
            edge_op(src_id, dst_id, global_edge_pos, vector_index);
        }
